...
const SwapButton = forwardRef(
  (
    { amount, destCurrency, destNetwork, destAddr, isSubmitting, tag }: Props,
    ref
  ) => {
    ...
    useEffect(() => {
      if (createState.status.toString() === "Mining") {
        toast({
          title: "Waiting",
          description:
            "Transaction is mining at this moment, soon it will be confirmed...",
          status: "info",
          duration: 9000,
          isClosable: true,
        });
        ...
        toast({
          title: "Confirmation",
          description:
            "Swap request was successfully sent.\nNow waiting for a deposit approval...",
          status: "success",
          duration: 9000,
          isClosable: true,
        });
    ...
    useImperativeHandle(ref, () => ({
      async onSubmit() {
        ...
        const filter = contract.filters.CostResponse(
          account,
          serviceAddress,
          productId
        );
